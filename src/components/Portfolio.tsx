...
export default function Portfolio({ projects }: PortfolioProps) {
  ...
  return (
    ...
    >
      ...
      {projects
        ?.filter((project) => [1, 2, 4].includes(project.id))
        .map((project) => (
          <div
            className={`flex flex-col md:flex-row md:even:flex-row-reverse gap-2 pb-12 border-b-2 border-dashed ${
          ...
          >
            ...
            <div className="w-full cursor-grab rounded-xl mt-2 self-center relative">
              ...
              >
                <Swiper
                  ...
                  className={`relative overflow-hidden w-[304px] md:w-[500px] border-4 rounded-2xl shadow-2xl ${
                    theme === "dark" ? "border-orange-500" : "border-blue-900"
                  }`}
                >
                  {project.imgs.map((img: string, index) => (
                    <SwiperSlide key={index}>
                      <Image
                        className={`rounded-xl duration-2000 border ${
                          theme === "dark"
                            ? "border-orange-500"
                            : "border-blue-900"
                        }`}
            ...
            <div className="flex flex-col pb-2 mt-6">
              ...
              <p
                className={`text-lg h-full px-2 pb-2 md:p-4 ${
                  theme === "dark" && "text-orange-200"
                }`}
              >
                {project.text}
              </p>
              <div
                className={`flex gap-10 self-center md:justify-self-end mt-2`}
              >
                <Link
                  target="_blank"
                  href={project.app}
